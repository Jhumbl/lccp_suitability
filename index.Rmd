---
title: "City Highways - Lamp Column Charger Suitability Analysis"
output: 
...
Full Overlay Map {data-icon="glyphicon-flash"}
=====================================  
...
columns %>%
  ...
  addCircles(data=dedicated_locations, lat=~Latitude, lng=~Longtitude, radius = 15, stroke=FALSE, fillColor = "#fffdd0", fillOpacity = 1, group="Dedicated Bays", popup=~Charge_Point) %>%
  addLegendCustom(colors = c("#6C3483", "#D4AC0D", "white", "#fffdd0"), labels = c("Non Convertible LC", "Potentially Convertible LC", "Existing LCCP", "Dedicated Bays"), sizes = c(15, 15, 20, 25)) %>%
  addLegend(position="bottomright", labels = c("Suitable LC Locations: 2,621", "Unsuitable LC Locations: 9,667", "Existing LC Charge Points: 313"), values="d", colors = c("black","black","black")) %>%
  addLayersControl(
    overlayGroups = c("Residential Parking Bays", "Dedicated Bays", "Lamp Columns", "Lamp Column Chargers"),
    position="bottomleft",
    options = layersControlOptions(collapsed = FALSE, autoZIndex = TRUE)
  ) %>%
  hideGroup(c("Residential Parking Bays", "Dedicated Bays")) %>%
  addSearchOSM() 
```
